...
基於 Selenium 的自動化工具，旨在解決不同設備（如 AVM5544）需要反覆登入以產生系統 Log 的需求。透過模組化設計，讓測試腳本更易於維護與擴充。
...
5. **如何新增設備**：
   - 在 devices/ 目錄下建立新的類別。
   - 繼承 BaseDevice 並實作專屬的 login 與 logout 方法。
   - 在 main.py 中修改實例化的對象，例如：target_device = NewDevice(driver, wait, Config)
